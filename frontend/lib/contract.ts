...
const CONTRACT_ADDRESS = "ST3W3XEFCN8Q9BN3KK2B8CY5PP9DX6HSSMAESKVGA";
...
export async function getGame(gameId: number) {
  ...
  const game: Game = {
    ...
    forfeited: cvToValue(gameCV["forfeited"]),
  };
  return game;
}
...
export async function forfeitGame(gameId: number) {
  const txOptions = {
    contractAddress: CONTRACT_ADDRESS,
    contractName: CONTRACT_NAME,
...
export async function resetGame(gameId: number, newBetAmount: number) {
  const txOptions = {
    contractAddress: CONTRACT_ADDRESS,
    contractName: CONTRACT_NAME,
    functionName: "reset-game",
    functionArgs: [uintCV(gameId), uintCV(newBetAmount)],
  };
...
export async function getGameStatus(gameId: number) {
  try {
    ...
    const responseCV = statusResult as OptionalCV<
      TupleCV<{
        status: UIntCV;
        winner: OptionalCV<PrincipalCV>;
      }>
    ...
    const statusData = responseCV.value.value;
    const status = parseInt(statusData.status.value.toString());
    const winner =
      statusData.winner.type === "some" ? statusData.winner.value.value : null;

    return {
      status, // 0: ongoing, 1: won, 3: forfeited
      winner,
    };
  } catch (error) {
